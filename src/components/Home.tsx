...
export default function Home() {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">YouTube Comment Sentiment Analysis</h1>
      ...
      {error && (
        <div className="text-red-500 text-sm mb-4 p-3 bg-red-50 rounded-md">{error}</div>
      )}
        ...
        <Card className="p-4 mb-6">
          ...
          <div className="max-h-60 overflow-y-auto space-y-2">
            {comments.map((comment, index) => (
              <p key={index} className="text-gray-600 p-2 bg-gray-50 rounded">{comment}</p>
            ))}
          </div>
